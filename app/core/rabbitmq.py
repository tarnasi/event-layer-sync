...
class RabbitMQConnection:
    ...
    def connect(self):
        try:
            ...
            # Declare exchange and queues for distributed system
            self.channel.exchange_declare(exchange='distributed_events', exchange_type='topic')
            
            # Create queues for each server
            for server_id in ["A", "B", "C", "D"]:
                queue_name = f"server_{server_id}_events"
                self.channel.queue_declare(queue=queue_name, durable=True)
                # Bind to receive events from other servers
                self.channel.queue_bind(
                    exchange='distributed_events', 
                    queue=queue_name, 
                    routing_key=f"*.{server_id}"  # Events targeted to this server
                )
    ...
    def publish_distributed_event(self, event_type: str, url: str, method: str, 
        ...
        try:
            # Send event to all other servers (not to self)
            target_servers = [s for s in settings.ALLOWED_SERVERS if s != settings.SERVER_ID]
            
            for target_server in target_servers:
                routing_key = f"{settings.SERVER_ID}.{target_server}"  # from.to
                
                message = {
                    "source_server": settings.SERVER_ID,
                    "target_server": target_server,
                    "event_type": event_type,  # warehouse.created, shipment.updated, etc.
